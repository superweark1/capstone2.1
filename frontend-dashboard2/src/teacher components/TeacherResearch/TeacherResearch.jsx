...
const TeacherResearch = ({ researchId }) => {
  ...
  const handleUpdate = async () => {
    ...
    if (fileToUpload) {
      // Assuming `fileToUpload` is the file object you want to upload
      const formData = new FormData();
    ...
    try {
      const response = await axios.put(
        `${process.env.REACT_APP_API_URL}/update/${fileIdToUpdate}`,
        {
          title,
          author,
          year,
  ...
  const downloadFile = async (filename) => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/download/${filename}`,
        {
          responseType: "blob",
        }
      );
  ...
  const filteredFiles = files.filter((file) =>
    file.file_research.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const handleView = async (fileId) => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/viewfile/${fileId}`
      );
    ...
    if (window.confirm("Are you sure you want to delete this file?")) {
      try {
        const response = await axios.delete(
          `${process.env.REACT_APP_API_URL}/delete/${fileId}`
        );
        alert(response.data.message); // Show success message
        fetchFiles(); // Fetch the updated list of files
      } catch (error) {
                ...
                currentFiles.map((file) => (
                  <tr key={file.id}>
                    ...
                    <td>
                      <a
                        href="#"
                        onClick={() => downloadFile(file.file_research)}
                      >
                        {file.file_research}
                      </a>
                    </td>
