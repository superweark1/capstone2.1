...
const Teacher = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:5001/teacher')
      .then(res => {
  ...
  const filteredTeachers = teachers.filter(teacher =>
    (teacher.fname && teacher.fname.toLowerCase().includes(searchQuery.toLowerCase())) ||
    (teacher.lname && teacher.lname.toLowerCase().includes(searchQuery.toLowerCase())) ||
    (teacher.gender && teacher.gender.toLowerCase().includes(searchQuery.toLowerCase())) ||
    (teacher.email && teacher.email.toLowerCase().includes(searchQuery.toLowerCase())) ||
    (Array.isArray(teacher.expertise) && teacher.expertise.some(exp => exp.toLowerCase().includes(searchQuery.toLowerCase()))) || 
  ...
  const handleSubmit = (event) => {
    ...
    const url = editMode ? `http://localhost:5001/teacher-update/${selectedTeacher.id}` : 'http://localhost:5001/teacher-create';
    ...
    axios[method](url, teacherData)
      .then(res => {
        ...
        if (editMode) {
          ...
          if (expertise !== selectedTeacher.expertise) updatedFields.push('Expertise');
          
          message += updatedFields.length > 0 ? ` Updated fields: ${updatedFields.join(', ')}.` : '';
        } else {
          // Add message
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this teacher?')) {
      axios.delete(`http://localhost:5001/teacher-delete/${id}`)
        .then(res => {
  ...
  const handleStatusChange = (event, id) => {
    ...
    axios.put(`http://localhost:5001/teacher-update/${id}`, {
      status: newStatus
    })
    .then((res) => {
        ...
        >
          <Box sx={style}>
            <div className="teacher-input-container">
              <h1 className='add'>{editMode ? 'Edit Teacher' : 'Add Teacher'}</h1>
                ...
                >
                  {[
                    ...
                    <option key={expertiseOption} value={expertiseOption}>
                      {expertiseOption.replace(/-/g, " ").replace(/\b\w/g, (char) => char.toUpperCase())}
                    </option>
                  ))}
                </select>
              </div>
