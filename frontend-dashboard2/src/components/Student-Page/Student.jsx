...
const Student = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:5001/student-list')
      .then(res => {
  ...
  const filteredStudents = students.filter(student =>
    (student.student_id && String(student.student_id).includes(searchQuery)) ||
    (student.fname && student.fname.toLowerCase().includes(searchQuery.toLowerCase())) ||
    (student.lname && student.lname.toLowerCase().includes(searchQuery.toLowerCase())) ||
    (student.gender && student.gender.toLowerCase().includes(searchQuery.toLowerCase())) ||
    (student.year_level && student.year_level.toLowerCase().includes(searchQuery.toLowerCase())) ||
    (student.email && student.email.toLowerCase().includes(searchQuery.toLowerCase())) ||
    (student.status && student.status.toLowerCase().includes(searchQuery.toLowerCase()))
  );
  ...
  const handleSubmit = (event) => {
    ...
    const url = editMode ? `http://localhost:5001/student-update/${selectedStudent.id}` : 'http://localhost:5001/student-create';
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this student?')) {
      axios.delete(`http://localhost:5001/student-delete/${id}`)
        .then(res => {
              ...
              {currentStudents.map(student => (
                <tr key={student.id}>
                  ...
                  <td>
                    ...
                    <Button onClick={() => handleDelete(student.id)} variant="outlined" color="secondary">
                      Delete
                    </Button>
                  </td>
                </tr>
