...
const AssignStudent = () => {
  ...
  const filteredGroups = groups.filter((group) => {
    ...
    return (
      ...
      (group.student_names && group.student_names.toLowerCase().includes(searchTerm)) ||
  ...
  const handleSubmit = async () => {
    try {
      ...
      const panelMembersArray = Array.isArray(panel_members) ? panel_members : [];
      ...
      const panelmembersEmails = teachers
        .filter(teacher => panelMembersArray.includes(`${teacher.fname} ${teacher.lname}`))
...
const filteredTeachers = teachers.filter(teacher => `${teacher.fname} ${teacher.lname}` !== adviser);
