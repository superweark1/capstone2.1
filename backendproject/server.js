...
app.put('/mark-notification-read/:id', (req, res) => {
    ...
    db.query(sql, [id], (err, result) => {
      if (err) {
        ...
        const countSql = 'SELECT COUNT(*) AS unreadCount FROM notifications WHERE is_read = FALSE';
  ...
  app.get('/unread-notification-count', (req, res) => {
    const sql = 'SELECT COUNT(*) AS unreadCount FROM notifications WHERE is_read = FALSE';
...
app.post('/upload', (req, res) => {
    ...
    file.mv(uploadPath, (err) => {
        ...
        const sql = 'INSERT INTO research (title, author, year, file_research) VALUES (?, ?, ?, ?)';
        db.query(sql, [title, author, year, file.name], (error, results) => {
            if (error) {
                return res.status(500).json({ error });
            }
            res.status(200).json({ message: 'File uploaded successfully', file_name: file.name });
        });
    });
});
...
app.get('/viewFile/:id', (req, res) => {
    ...
    const sql = 'SELECT title, author, year, file_research FROM research WHERE id = ?';
...
app.delete('/delete/:id', (req, res) => {
    ...
    db.query('SELECT file_research FROM research WHERE id = ?', [id], (err, results) => {
        if (err) return res.status(500).json({ message: 'Error fetching file', error: err });
        ...
        fs.unlink(filePath, (err) => {
            if (err) return res.status(500).json({ message: 'Error deleting file from filesystem', error: err });

            // Then delete the record from the database
            db.query('DELETE FROM research WHERE id = ?', [id], (err, result) => {
                if (err) return res.status(500).json({ message: 'Error deleting file record', error: err });
...
const verifyUser = (req, res, next) => {
    ...
    jwt.verify(token, process.env.JWT_SECRET || 'new-secret-key', (err, decoded) => {
        if (err) {
...
app.post('/register', async (req, res) => {
    const { firstname, lastname, email, role, birthdate, gender, password } = req.body;

    // Validate input fields
    if (!firstname || !lastname || !email || !role || !birthdate || !gender || !password) {
        return res.status(400).json({ message: "All fields are required." });
    }
    ...
    db.query('INSERT INTO user SET ?', user, async (err, result) => {
        ...
        try {
            ...
            return res.status(500).json({ message: 'User registered successfully, but failed to send email to user.' });
        }
    ...
    db.query('SELECT * FROM user WHERE email = ?', [email], (err, result)=> {
        ...
        if(result.length > 0) {
            bcrypt.hash(password, 10, (hashErr, hashedPassword) => {
                ...
                db.query('UPDATE user SET password = ? WHERE email = ?', [hashedPassword, email], (updateErr) => {
                    if(updateErr) {
...
app.post('/create-group', async (req, res) => {
  try {
    ...
    const { 
      ...
      student_emails, panel_head, panel_members, panelhead_email, panelmembers_email, grade, recommendation, revision_type 
    } = req.body;
    ...
    await db.promise().query(query, [
      group_name, name, research_title, adviser, adviser_email, 
      JSON.stringify(student_emails), panel_head, JSON.stringify(panel_members), 
      panelhead_email, JSON.stringify(panelmembers_email), grade, recommendation, revision_type
    ]);
...
app.get('/student-list', (req, res) => {
  ...
  app.post('/student-create', (req, res) => {
    const { student_id, fname, lname, gender, year_level, email, status } = req.body;
  
    if (!student_id || !fname || !lname || !gender || !year_level || !email || !status) {
      return res.status(400).json({ message: 'All fields are required' });
    }
  
    const query = 'INSERT INTO student_list (student_id, fname, lname, gender, year_level, email, status) VALUES (?, ?, ?, ?, ?, ?, ?)';
    db.query(query, [student_id, fname, lname, gender, year_level, email, status], (err, result) => {
      if (err) {
  ...
  app.put('/student-update/:id', (req, res) => {
    ...
    db.query(query, [student_id, fname, lname, gender, year_level, email, status, id], (err, result) => {
      if (err) {
...
app.get('/teacher', (req, res) => {
    const query = 'SELECT id, fname, lname,teacher_id,gender,email,expertise,status FROM teacher_list';
  ...
  app.post('/teacher-create', (req, res) => {
    const { fname, lname, teacher_id, gender, email, status, expertise } = req.body;
  
    if (!fname || !lname || !teacher_id || !gender || !email || !status || !expertise) {
        return res.status(400).json({ error: 'All fields are required' });
    }

    // If expertise is an array, join the items into a comma-separated string
    const expertiseString = Array.isArray(expertise) ? expertise.join(', ') : expertise;

    const sql = `INSERT INTO teacher_list (fname, lname, teacher_id, gender, email, status, expertise) VALUES (?, ?, ?, ?, ?, ?, ?)`;
    const values = [fname, lname, teacher_id, gender, email, status, expertiseString];
...
app.put('/teacher-update/:id', (req, res) => {
    ...
    const { fname, lname, teacher_id, gender, email, status, expertise } = req.body;  // Include expertise
    ...
    const values = [fname, lname, teacher_id, gender, email, status, expertiseString, teacherId];
...
app.put('/update-group/:id', (req, res) => {
    ...
    const panel_members_string = Array.isArray(panel_members) ? panel_members.join(', ') : panel_members;
    
    // Ensure panel_members_email is a string (if it's an array, join them with commas)
    const panel_members_email_string = Array.isArray(panelmembers_email) ? panelmembers_email.join(', ') : panelmembers_email;
...
app.post('/create-scheduler', (req, res) => {
    ...
    const getEmailQuery = 'SELECT adviser_email, student_emails, panelhead_email, panelmembers_email FROM asign WHERE group_name = ?';
    const sql = `
    ...
    const promises = groups.map((group) => {
        return new Promise((resolve, reject) => {
            db.query(getEmailQuery, [group.group_name], (err, results) => {
                ...
                const studentEmails = results[0]?.student_emails ? results[0].student_emails.split(',') : [];
                const panelHeadEmail = results[0] ? results[0].panelhead_email : null;
                const panelMembersEmails = results[0]?.panelmembers_email ? results[0].panelmembers_email.split(',') : [];
    ...
    Promise.all(promises)
        ...
        .catch((error) => res.status(500).json({ message: 'Error creating schedule', error }));
});
...
app.put('/api/data/:id', (req, res) => {
    ...
    const updateQuery = 'UPDATE scheduler SET name = ?, start_date = ?, start_time = ?, end_time = ? WHERE id = ?';
    ...
    function formatTime(time) {
        ...
        return new Date(0, 0, 0, hour, minute).toLocaleTimeString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true });
    }
    ...
    db.query(updateQuery, [name, start_date, start_time, end_time, id], (err, results) => {
        if (err) {
            console.error('Error updating data:', err);
            return res.status(500).send('Server error');
        }
        ...
        db.query(getDetailsQuery, [id], (err, detailsResults) => {
            ...
            const {
                adviser_email, panelhead_email, panelmembers_email, student_emails,
                research_title, group_name, name, adviser, panel_head, panel_members
            } = detailsResults[0];

            const panelMembersEmails = panelmembers_email ? panelmembers_email.split(',') : [];
        ...
        Scheduler Team`;
            ...
            if (panelhead_email) {
                ...
                transporter.sendMail(mailOptionsPanelHead, (error, info) => {
                    if (error) console.error('Error sending email to panel head:', error);
            ...
            panelMembersEmails.forEach(panelMemberEmail => {
                if (panelMemberEmail) {
                    ...
                    transporter.sendMail(mailOptionsPanelMember, (error, info) => {
                        if (error) console.error('Error sending email to panel member:', error);
            ...
            studentEmails.forEach(studentEmail => {
                if (studentEmail) {
                    ...
                    transporter.sendMail(mailOptionsStudent, (error, info) => {
                        if (error) console.error('Error sending email to student:', error);
    ...
    db.query('SELECT * FROM scheduler', (err, results) => {
        ...
        const events = results.map((event) => {
            ...
            if (isNaN(startDateTime.getTime()) || isNaN(endDateTime.getTime())) {
                console.error('Invalid date:', event.start_date, event.start_time, event.end_time);
            }
...
app.put('/api/assign', (req, res) => {
    ...
    db.query(query, [grade, recommendation, revision_type, group_name], (err, result) => {
      if (err) {
  ...
  app.post('/api/grades1', (req, res) => {
    ...
    if (!panel_1_grades && !panel_2_grades && !panelhead_grades) {
        return res.status(400).json({ error: 'At least one grade field (panel_1_grades, panel_2_grades, panelhead_grades) is required' });
    }

    // Check if revision_type is required for panelhead_grades
    if (panelhead_grades && !revision_type) {
        return res.status(400).json({ error: 'Revision type is required for panel head grades' });
    }
    ...
    db.query(checkGroupQuery, [group_name], (err, result) => {
        ...
        if (result.length > 0) {
            ...
            db.query(updateQuery, updateValues, (err, result) => {
                ...
                return res.status(200).json({ message: 'Grade updated successfully', result });
            });
        } else {
            ...
            db.query(insertQuery, insertValues, (err, result) => {
                ...
                return res.status(200).json({ message: 'Grade inserted successfully', result });
            });
        }
    });
});
